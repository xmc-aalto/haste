...
DATASET_NAME_MAP = {'eurlex4k': 'Eurlex-4K','wiki31k': 'Wiki10-31K', 'amazon670k': 'Amazon-670K', 'wiki500k': 'Wiki-500K',
                ...
                'lfwikiseealso320k':'LF-WikiSeeAlso-320k','amazontitles670k':'AmazonTitles-670K'}
...
DATASET_TO_PATH_OBJECT = {'eurlex4k': PathEurlex4K, 'wiki31k': PathWiki31K, 'amazon670k': PathAmazon670K,'wiki500k' : PathWiki500K,  
            ...
            'lfwikiseealso320k':PathLFWikiSeeAlso320K,'amazontitles670k':PathAmazonTitles670K}
...
def main(cfg: SimpleConfig) -> None:
    ...
    f = open(path.train_raw_texts)
    data = []
    for f in f.readlines():
        data.append(f.strip())

    f = open(path.train_labels)
    labels = []
    for f in f.readlines():
        labels.append(f.strip())

    f = open(path.test_labels)
    test_labels = []
    for f in f.readlines():
        test_labels.append(f.strip())

    train_all_labels = [lbl.split() for lbl in labels]
    test_all_labels = [lbl.split() for lbl in test_labels]

    train_all_labels  = list(itertools.chain.from_iterable(train_all_labels))
    test_all_labels = list(itertools.chain.from_iterable(test_all_labels))
    ...
    if len(only_test_labels) > 0:
        ...
        f = open(path.test_raw_texts)
        test_data = []
        for f in f.readlines():
            test_data.append(f.strip())

        for i,d in tqdm(enumerate(test_data)):
            labels_list = test_labels[i].split()
            for lbl in labels_list:
